...
# coding: utf-8
"""
ReviewCash — combined Flask + TeleBot + SocketIO server.

Features:
- Telegram bot handlers: /start (with channel subscription check), /support flow, /admin, /mainadmin (opens WebApp via web_app button), /addadmin
- Public API endpoints for creating topup and withdraw requests (with validations, OTP for topup)
- Admin endpoints protected by short-lived JWT tokens (approve/reject topups and withdraws, manage supports, list admins)
- User profile endpoint that uses Telegram WebApp init_data and tries to return avatar URL
- Combined user history endpoint (topups, withdraws, works)
- SocketIO notifications for admin UIs
- Data persisted into JSON files under DATA_DIR
Notes:
- This example includes BOT_TOKEN and WEBAPP_URL fallback to environment variables. You may set them via environment for production.
"""
...
# ----------------- CONFIG -----------------
# You can set these via environment variables. If not set, defaults below are used.
...
CHANNEL_ID = os.environ.get("CHANNEL_ID", "@YourChannel")  # set your required channel here

# Main admins (IDs or usernames)
...
ADMIN_TOKEN_TTL_SECONDS = int(os.environ.get("ADMIN_TOKEN_TTL_SECONDS", "600"))  # default 10 min
...
# ----------------- Bot Handlers -----------------
if bot:
    # /start with subscription request and check
    @bot.message_handler(commands=['start'])
    def handle_start(message):
        try:
            uid = message.from_user.id
            first = (message.from_user.first_name or "").strip()
            text = f"Привет{(' ' + first) if first else ''}! Это ReviewCash.\n\n" \
                   "Важно: для работы требуется подписка на наш канал."
            ...
            # open webapp button
            try:
                admin_url = WEBAPP_URL.rstrip('/') + '/'
                kb.add(tb_types.InlineKeyboardButton(text="Открыть WebApp", url=admin_url))
            except Exception:
                pass
    ...
    def cb_check_sub(call):
        try:
            ...
            if subscribed:
                ...
                bot.send_message(uid, f"Подпишитесь на {CHANNEL_ID} и нажмите Проверить подписку ещё раз.")
        except Exception as e:
            logger.exception("callback check_sub error: %s", e)

    # admin commands: /admin, /mainadmin, /addadmin
    @bot.message_handler(commands=['admin', 'mainadmin', 'addadmin'])
    def handle_admin_cmds(message):
        try:
            ...
            if cmd == '/admin':
                ...
                return
            if cmd == '/mainadmin':
                # generate admin token and send WebApp button (web_app param)
            ...
            if cmd == '/addadmin':
                ...
                if added:
                    bot.send_message(uid, f"{target} добавлен(а) в список ordinary admins.")
                    socketio.emit('admins_updated', {"action": "add", "who": target}, room='admins_main')
                else:
                    bot.send_message(uid, f"{target} уже есть в списке админов.")
                return
        except Exception as e:
            logger.exception("admin cmd error: %s", e)

    # support flow: /support then next message
    _awaiting_support = {}

    @bot.message_handler(commands=['support'])
    def handle_support_prompt(message):
        try:
            bot.send_message(message.chat.id, "Опишите проблему — отправьте сообщение, которое станет вашим обращением в поддержку.")
    ...
    def handle_generic_messages(message):
        try:
            ...
            if _awaiting_support.get(cid):
                ...
                socketio.emit('new_support', rec, room='admins_main')
                # notify admins via bot
                for admin in ordinary_admins + ADMIN_USER_IDS:
                    try:
                        if str(admin).isdigit():
                            bot.send_message(int(admin), f"Новый запрос в поддержку от {cid}: {text[:300]}")
                        else:
                            bot.send_message(f"@{admin}", f"Новый запрос в поддержку: {text[:300]}")
                    except Exception:
                        pass
                bot.send_message(cid, "Спасибо — ваше сообщение отправлено в поддержку.")
                return
            # otherwise echo
            bot.reply_to(message, "Спасибо, сообщение получено.")
        except Exception as e:
            logger.exception("generic handler error: %s", e)
...
# token retrieval helpers for admin endpoints
...
# ----- profile_me: returns user info and photo url when possible -----
...
def api_profile_me():
    ...
    username = params.get('username') or params.get('user') or None
    rec = get_user_record(uid)
    result = {"ok": True, "user": {"id": uid, "username": username, "first_name": params.get('first_name')}, "balance": rec.get('balance', 0.0), "subscribed": rec.get('subscribed', False)}
    # try to fetch avatar via bot
    if bot:
        try:
            photos = bot.get_user_profile_photos(int(uid))
            if photos and photos.total_count > 0:
                # choose first size of first photo
...
def api_topups_public():
    ...
    if bot:
        try:
            admin_msg = f"Новый топап: {amount} ₽, код {code}, user: {user.get('id') or 'unknown'}"
            for a in ordinary_admins + ADMIN_USER_IDS:
                try:
                    if str(a).isdigit():
                        bot.send_message(int(a), admin_msg)
                    else:
                        bot.send_message(f"@{a}", admin_msg)
                except Exception:
                    pass
        except Exception:
            pass
    bank_info = {"bank_name": "Тинькофф (Раяз Н.)", "phone": "+79600738559", "note": "В комментарии укажите код"}
    return jsonify({"ok": True, "topup": rec, "bank": bank_info})

# ----- withdraw create (public) -----
...
def api_withdraw_public():
    ...
    if bot:
        try:
            admin_msg = f"Новый вывод: {amount} ₽, user {user_id}, {name}, {bank}"
            for a in ordinary_admins + ADMIN_USER_IDS:
                try:
                    if str(a).isdigit():
                        bot.send_message(int(a), admin_msg)
...
# ----- admin endpoints for topups/withdraws (approve/reject) -----
...
def api_topup_approve(tid):
    ...
    if bot and user_id:
        try:
            bot.send_message(int(user_id), f"Пополнение {found.get('amount')} ₽ подтверждено. Баланс обновлён.")
        except Exception:
            pass
...
def api_topup_reject(tid):
    ...
    if bot and user_id:
        try:
            bot.send_message(int(user_id), f"Пополнение отклонено: {reason}")
        except Exception:
            pass
    return jsonify({"ok": True, "topup": found})
...
def api_withdraw_approve(wid):
    ...
    if bot and user_id:
        try:
            bot.send_message(int(user_id), f"Выплата {found.get('amount')} ₽ по заявке {wid} отмечена как выполненная.")
        except Exception:
            pass
...
def api_withdraw_reject(wid):
    ...
    if bot and user_id:
        try:
            bot.send_message(int(user_id), f"Ваша заявка на вывод отклонена. {reason}. Сумма возвращена на баланс.")
        except Exception:
            pass
    return jsonify({"ok": True, "withdraw": found, "balance": get_user_record(user_id)['balance'] if user_id else None})

# ----- support endpoints (list/reply/resolve) -----
...
# ----- user_history endpoint (combined) -----
...
def start_polling_thread_safe():
    if os.environ.get("BOT_DISABLE_BOT", "").lower() in ("1", "true", "yes"):
        logger.info("BOT_DISABLE_BOT set — skipping bot")
        return None
    if not bot:
        logger.info("bot not configured")
        return None
    try:
        bot.remove_webhook()
    except Exception:
    ...
    if not can_resolve_host():
        logger.warning("api.telegram.org not resolvable — skipping polling")
        return None
    def loop():
        while True:
            try:
...
def setup_webhook_safe():
    ...
    try:
        ...
        logger.info("Webhook set: %s", f"{WEBAPP_URL.rstrip('/')}/webhook")
    except Exception as e:
...
if __name__ == '__main__':
    # start bot webhook/polling in background
